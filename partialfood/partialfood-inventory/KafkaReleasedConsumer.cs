...
namespace PartialFoods.Services.InventoryServer
{
    ...
    public class KafkaReleasedConsumer
    {
        ...
        private readonly InventoryReleasedEventProcessor eventProcessor;
                ...
                using (var consumer = new Consumer<Null, string>(this.config, null, new StringDeserializer(Encoding.UTF8)))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (consumer.Consume(out msg, TimeSpan.FromSeconds(1)))
                        {
                            ...
                            try
                            {
                                var evt = JsonConvert.DeserializeObject<InventoryReleasedEvent>(rawJson);
                                this.eventProcessor.HandleInventoryReleasedEvent(evt);
                            ...
                            catch (Exception ex)
                            {
                                ...
                                Console.WriteLine($"Failed to handle inventory released event : ${ex}");
                            }
                        }
                    }
                }
